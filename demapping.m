...
% - modulation : 'pam' or 'qam' or 'qpsk' or 'bpsk'
%
% OUTPUTS:
...
switch modulation,
    ...
    case 'qpsk'
        % Symbol to integer
        sigma = log(symb_rx)/(1i*pi);
        int_rx = (2^Nbps-2)*(sigma-3/4);
        ...
        % Binary to gray
        bit_rx2 = bin2gray(mapp_rx,'psk',2^Nbps);
        bit_rx = reshape(bit_rx2',Nsymb*Nbps,1);
        
        %bit_rx=pskdemod(symb_rx,4);
        
    case 'bpsk' % In case of bpsk, the Npbs must be 1
        %Symbol to integer
        sigma = log(symb_rx)/(1i*pi);
        int_rx = 2^(Nbps-1)*(sigma-3/4);
        ...
        % Binary to gray
        bit_rx2 = bin2gray(mapp_rx,'psk',2);
        bit_rx = reshape(bit_rx2',Nsymb*Nbps,1);

        %bit_rx=pskdemod(symb_rx,2);
